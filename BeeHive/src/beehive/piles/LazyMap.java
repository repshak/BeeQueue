...
import beehive.util.Morph;
import beehive.util.PassThruTransformation;
...
public class LazyMap<K, V> extends LinkedHashMap<K, V> {
  ...
  public static <K,V> LazyMap<K,V> extractKeys(Morph<V,K> extractKey, Collection<V> collection){
  ...
  public static <K2,K,V> LazyMap<K,V> morphKeys(Morph<K2,K> morphKey, Map<K2,V> toMorph){
    return new LazyMap<K,V>().morphKeysInto(morphKey,toMorph);
  }

  
  public static <V2,K,V> LazyMap<K,V> morphValues(Morph<V2,V> morphValue, Map<K,V2> toMorph){
    return new LazyMap<K,V>().morphValuesInto(morphValue,toMorph);
  }

  public static <K2,V2,K,V> LazyMap<K,V> morph(Morph<K2,K> morphKey, Morph<V2,V> morphValue, Map<K2,V2> toMorph){
    return new LazyMap<K,V>().morphInto(morphKey,morphValue,toMorph);
  }

  public <K2> LazyMap<K,V> morphKeysInto(Morph<K2,K> morphKey, Map<K2,V> toMorph){
    final PassThruTransformation<V> passThruValue = new PassThruTransformation<V>();
    return morphInto(morphKey,passThruValue,toMorph);
  }
  
  public <V2> LazyMap<K,V> morphValuesInto(Morph<V2,V> morphValue, Map<K,V2> toMorph){
    final PassThruTransformation<K> passThruKey = new PassThruTransformation<K>();
    return morphInto(passThruKey,morphValue,toMorph);
  }

  public <K2,V2> LazyMap<K,V> morphInto(Morph<K2,K> morphKey, Morph<V2,V> morphValue, Map<K2,V2> toMorph){
